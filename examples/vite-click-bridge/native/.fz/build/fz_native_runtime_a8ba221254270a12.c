...
typedef struct BridgeClickResult {
  int32_t input_count;
  int32_t js_doubled;
  int32_t callback_total;
  int32_t handshake_score;
  uint64_t module_handle;
  uint64_t double_export;
  uint64_t input_handle;
  uint64_t pending_handle;
  uint64_t settled_handle;
  uint64_t registration_handle;
  uint64_t host_module;
  uint64_t host_export;
  uint64_t callback_pending;
  uint64_t callback_settled;
} BridgeClickResult;

static uint64_t fz_encode_BridgeClickResult(BridgeClickResult value) {
  uint64_t handle = fz_native_agg_new(0, 14);
  if (handle == 0) {
    return 0;
  }
  (void)fz_native_agg_set_i64(handle, 0, (uint64_t)value.input_count);
  (void)fz_native_agg_set_i64(handle, 1, (uint64_t)value.js_doubled);
  (void)fz_native_agg_set_i64(handle, 2, (uint64_t)value.callback_total);
  (void)fz_native_agg_set_i64(handle, 3, (uint64_t)value.handshake_score);
  (void)fz_native_agg_set_i64(handle, 4, (uint64_t)value.module_handle);
  (void)fz_native_agg_set_i64(handle, 5, (uint64_t)value.double_export);
  (void)fz_native_agg_set_i64(handle, 6, (uint64_t)value.input_handle);
  (void)fz_native_agg_set_i64(handle, 7, (uint64_t)value.pending_handle);
  (void)fz_native_agg_set_i64(handle, 8, (uint64_t)value.settled_handle);
  (void)fz_native_agg_set_i64(handle, 9, (uint64_t)value.registration_handle);
  (void)fz_native_agg_set_i64(handle, 10, (uint64_t)value.host_module);
  (void)fz_native_agg_set_i64(handle, 11, (uint64_t)value.host_export);
  (void)fz_native_agg_set_i64(handle, 12, (uint64_t)value.callback_pending);
  (void)fz_native_agg_set_i64(handle, 13, (uint64_t)value.callback_settled);
  return handle;
}

static BridgeClickResult fz_decode_BridgeClickResult(uint64_t handle) {
  BridgeClickResult value = {0};
  if (handle == 0) {
    return value;
  }
  value.input_count = (int32_t)fz_native_agg_get_i64(handle, 0);
  value.js_doubled = (int32_t)fz_native_agg_get_i64(handle, 1);
  value.callback_total = (int32_t)fz_native_agg_get_i64(handle, 2);
  value.handshake_score = (int32_t)fz_native_agg_get_i64(handle, 3);
  value.module_handle = (uint64_t)fz_native_agg_get_i64(handle, 4);
  value.double_export = (uint64_t)fz_native_agg_get_i64(handle, 5);
  value.input_handle = (uint64_t)fz_native_agg_get_i64(handle, 6);
  value.pending_handle = (uint64_t)fz_native_agg_get_i64(handle, 7);
  value.settled_handle = (uint64_t)fz_native_agg_get_i64(handle, 8);
  value.registration_handle = (uint64_t)fz_native_agg_get_i64(handle, 9);
  value.host_module = (uint64_t)fz_native_agg_get_i64(handle, 10);
  value.host_export = (uint64_t)fz_native_agg_get_i64(handle, 11);
  value.callback_pending = (uint64_t)fz_native_agg_get_i64(handle, 12);
  value.callback_settled = (uint64_t)fz_native_agg_get_i64(handle, 13);
  return value;
}

extern uint64_t bridge_click__fz_ffi_impl(int32_t count);
BridgeClickResult bridge_click(int32_t count) {
  uint64_t result_handle = bridge_click__fz_ffi_impl(count);
  BridgeClickResult result = fz_decode_BridgeClickResult(result_handle);

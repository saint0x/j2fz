...
static const int fz_string_literal_count = 3;
...
static uint64_t fz_encode_BridgeClickResult(BridgeClickResult value) {
  ...
  (void)fz_native_agg_set_i64(handle, 2, (uint64_t)value.native_lifted);
...
static BridgeClickResult fz_decode_BridgeClickResult(uint64_t handle) {
  ...
  value.native_lifted = (int32_t)fz_native_agg_get_i64(handle, 2);
